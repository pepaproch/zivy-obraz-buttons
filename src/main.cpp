...
//TODO - change to your own MAC address
const String addrA = "05:1a:d9:cc:4d:c0";
const String addrB = "05:f2:a5:4a:2d:15";
const String addrC = "05:55:00:1b:f4:9e";
...
int progressPos = 0;
int progressMax = 20;
char progressText[21] = "                    ";

void progress(int i1) {
    progressPos = progressPos + i1;
    Serial.println("progress");
    Serial.println(progressPos);
    if (progressPos >= progressMax) {
        progressPos = 0;
    }

    // Ensure the loop only fills up to progressMax - 1, leaving space for the null terminator
    ...
    // Add a null terminator to the end of the progressText
    progressText[progressMax] = '\0';

    display.setTextColor(GxEPD_BLACK);
    // display.fillRect(200, 200, 300, 40, GxEPD_BLACK  );
    display.drawRect(250, 80, 300, 40, GxEPD_BLACK);
    display.setCursor(250 + 10, 110);
    display.print(progressText);
    display.display(true);
}

void clerProggres() {
    display.fillRect(249, 79, 302, 42, GxEPD_WHITE);
    display.display(true);
}
        ...
        if ((planes == 1) && ((format == 0) || (format == 3))) // uncompressed is handled, 565 also
        {
            ...
            //if (w <= max_row_width) // handle with direct drawing
            {
                ...
                if (depth <= 8) {
                    ...
                    bytes_read += skip(client, (int32_t) (imageOffset - (4 << depth) -
                    ...
                    for (uint16_t col = 0; col < w; col++) // for each pixel
                    {
                        ...
                        switch (depth) {
                            ...
                            case 24:
                                ...
                                whitish = with_color ? ((red > 0x80) && (green > 0x80) && (blue > 0x80)) : (
                                        (red + green + blue) > 3 * 0x80); // whitish
                                lightgrey = with_color ? ((red > 0x60) && (green > 0x60) && (blue > 0x60)) : (
                                        (red + green + blue) > 3 * 0x60); // lightgrey
                                darkgrey = with_color ? ((red > 0x40) && (green > 0x40) && (blue > 0x40)) : (
                                        (red + green + blue) > 3 * 0x40); // darkgrey
                                colored = (red > 0xF0) || ((green > 0xF0) && (blue > 0xF0)); // reddish or yellowish?
...
#define BUTTON_PIN_BITMASK 0xB00000000 // GPIOs 35,33,32

uint get_wakeup_reason() {
    ...
    return static_cast<unsigned int>(wakeup_reason);
}

double print_GPIO_wake_up() {
...
RTC_DATA_ATTR int wakeUpPin = 35;

void setPaperMacAddr() {
    ...
    switch (wakeUpPin) {
        ...
        case 33:
            ...
            break;
        default:
            papewrMacAddr = WiFi.macAddress();
            break;
    }

    Serial.println("Using MAC address: " + papewrMacAddr);
}

void renderButtons() {
    uint w = 50;
    uint h = 40;
    uint x = display.width() - w;

    Serial.println("Displaying buttons");
    Serial.println(wakeUpPin);
    display.setFont(&OpenSansSB_24px);
    display.setTextColor(GxEPD_WHITE);
    switch (wakeUpPin) {
        case 32:
            display.fillRect(x, 40, w, h, GxEPD_BLACK);
            display.setCursor(x + 10, 40 + 30);
            display.print("32");
            break;
        case 35:
            display.fillRect(x, 135, w, h, GxEPD_BLACK);
            display.setCursor(x + 10, 135 + 30);
            display.print("35");
            break;

        case 33:
            display.fillRect(x, 230, w, h, GxEPD_BLACK);
            display.setCursor(x + 10, 230 + 30);
            display.print("33");
            break;
...
void counter() {
    ...
    for (int counter = 10; counter >= 0; counter--) {
        ...
        sprintf(counterText, "%02d", counter);
        ...
        // Clear the previous number by drawing a rectangle over it
        display.fillRect(x + x1 - 30, y + y1 - 30, w + 60, h + 60, GxEPD_WHITE);

        display.setTextColor(GxEPD_BLACK);
        // Set the cursor to the calculated position
        display.setCursor(x + x1, y); // Adjust for baseline

        // Display the counter
        display.print(counterText);

        // Use the partial update feature to update the display
  ...
  // DEMO mode
    ...
    if(bootCount%3==0 || bootCount == 1)  {
       display.init();
        ...
        display.firstPage();
        Serial.println("Full refreshh");

            display.fillScreen(GxEPD_BLACK);
            display.fillRect(0, 0, 100, 90, GxEPD_BLACK);
            display.setTextColor(GxEPD_WHITE);
            display.setFont(&OpenSansSB_24px);
            centeredText("DEMO Mode", DISPLAY_RESOLUTION_X / 2, 28);
            display.setFont(&OpenSansSB_18px);
            centeredText("Push any button", DISPLAY_RESOLUTION_X / 2, 64);
            centeredText("Going to sleep in ", DISPLAY_RESOLUTION_X / 2, 124);
...
void setup() {
  ...
  M5.update();
    ...
    if (wakeUpReason == 3) {
        ...
        display.setTextColor(GxEPD_BLACK);
        display.setFont(&OpenSansSB_24px);
        centeredText("User invoked update", DISPLAY_RESOLUTION_X / 2, 28);
        //fake  progress
        Serial.println("Fake progress");
        progress(1);
    ...
    if (wakeUpReason == 3 || checkForNewTimestampOnServer(client)) { ;
        if (wakeUpReason == 3) {
        ...
        delay(50);
        display.setFont(&OpenSansSB_24px);
        centeredText("Downloading bitmap", DISPLAY_RESOLUTION_X / 2, 158);
        centeredText("Using mac: " + macAddr, DISPLAY_RESOLUTION_X / 2, 200);
        display.display(true);
        delay(50);
